...
const createNewTicketValid = (req, res, next) => {
    const schema = Joi.object({ subject: longStr.required(), message: message.required(), sender: shortStr.required() })
...
const replyValid = (req, res, next) => {
    const schema = Joi.object({ message:message.required(), sender: shortStr.required() })
    const value = schema.validate(req.body)
...
module.exports = { resetPassValid, updatePassValid, createNewTicketValid, replyValid }